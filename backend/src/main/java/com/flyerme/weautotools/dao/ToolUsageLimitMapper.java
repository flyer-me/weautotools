...
public interface ToolUsageLimitMapper extends BaseMapper<ToolUsageLimit> {
     ...
     * 查询所有启用的限制配置（包含工具信息）
     */
    @Select("SELECT tul.*, t.status, t.tool_name, t.tool_type " +
    ...
    @Select("SELECT tul.* FROM tool_usage_limits tul " +
            "INNER JOIN tools t ON tul.tool_id = t.id " +
            "WHERE t.tool_name = #{toolName} AND tul.user_type = #{userType} " +
            "AND tul.enabled = true AND tul.deleted = 0 AND t.deleted = 0")
    List<ToolUsageLimit> selectByToolNameAndUserType(String toolName, String userType);

    /**
     * 查询所有启用的限制配置
     */
    @Select("SELECT * FROM tool_usage_limits WHERE enabled = true AND deleted = 0 ORDER BY tool_id, user_type")
